...
struct Nibl {
  ...
  /* foreground and background color state */
  uint8_t fgr; uint8_t fgg; uint8_t fgb; uint8_t fga;
  uint8_t bgr; uint8_t bgg; uint8_t bgb; uint8_t bga;
  uint64_t t;
  uint8_t stopped;
  uint8_t fps;
  /* mouse state */
  int32_t mx; int32_t my; int32_t mxr; int32_t myr; int32_t mpressed;
  /* keyboard state */
  struct nibl_key keyboard[KEY_BUFFER_SIZE];
  /* userdata that we pass to draw and get from setup */
  void *userdata;
};
...
static int32_t
processEvents(struct Nibl *n)
{
  SDL_Event event;
  int32_t err;
  uint32_t framestart;
  uint32_t frameticks;
  int keyplace; /* placement for keyboard state array */
  n->mx = n->my = n->mxr = n->myr = n->mpressed = 0; /* reset event state */

  if (n->t == 0 && n->setup != NULL) { /* run setup on first frame */
  ...
  while (SDL_PollEvent(&event)) {
    if (event.type == SDL_KEYDOWN) {
      ...
      n->keyboard[keyplace].code = event.key.keysym.sym;
      n->keyboard[keyplace].counter++;
    }
...
struct Nibl *
nibl_new(void *(*setup)(struct Nibl *), int32_t (*draw)(struct Nibl *, void *)) {
...
void
nibl_destroy(struct Nibl *n)
{
  if (n) { free(n); }
}

void
nibl_start(struct Nibl *n)
{
...
int32_t
nibl_mouse(struct Nibl *n, int32_t *x, int32_t *y, int32_t *xrel, int32_t *yrel)
{
  if (x != NULL) { *x = n->mx; }
  if (y != NULL) { *y = n->my; }
...
int32_t
nibl_keyboard(struct Nibl *n, SDL_Keycode code)
{
  for (int i = 0; i < KEY_BUFFER_SIZE; i++) {
...
void
nibl_setfg(struct Nibl *n, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
  n->fgr = r; n->fgg = g; n->fgb = b; n->fga = a;
}

void
nibl_setbg(struct Nibl *n, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
  n->bgr = r; n->bgg = g; n->bgb = b; n->bga = a;
}

void
nibl_background(struct Nibl *n, uint8_t r, uint8_t g, uint8_t b)
{
  SDL_SetRenderDrawColor(n->renderer, r, g, b, 255);
  SDL_RenderClear(n->renderer);
}

void
nibl_line(struct Nibl *n, int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
...
void
nibl_triangle(struct Nibl *n, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3)
{
...
void
nibl_box(struct Nibl *n, int32_t x, int32_t y, int32_t h, int32_t w, int32_t fill)
{
...
static void
nibl_drawchar(struct Nibl *n, char c, int32_t x, int32_t y)
{
  ...
  if (count > 0) {
    SDL_SetRenderDrawColor(n->renderer, n->fgr, n->fgg, n->fgb, n->fga);
    SDL_RenderFillRects(n->renderer, rects, count);
  }
}

void
nibl_text(struct Nibl *n, char *s, int32_t x, int32_t y)
{
  int32_t cx = x;
  while (*s) {
